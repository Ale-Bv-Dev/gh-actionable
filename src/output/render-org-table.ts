...
function renderIssueBlock(evaluated: EvaluatedIssue): string[] {
  ...
  const labels = issue.labels.length > 0 ? issue.labels.join(LIST_SEPARATOR) : EMPTY_PLACEHOLDER
  const warningsText =
    warnings.length > 0
